...
class AboutDictionaries(Koan):
    ...
    def test_making_a_dictionary_from_a_sequence_of_keys(self):
        keys = ('red warrior', 'green elf', 'blue valkyrie', 'yellow dwarf', 'confused looking zebra')
        default_value = 42
        cards = {}.fromkeys(keys, default_value)

        self.assertEqual(len(keys), len(cards))
        self.assertEqual(default_value, cards['green elf'])
        self.assertEqual(default_value, cards['yellow dwarf'])
